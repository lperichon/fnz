class Contact < ActiveRecord::Base
  ...
  scope :all_students, joins("left outer join memberships on contacts.id = memberships.contact_id").where("(padma_status = 'former_student' OR padma_status = 'student') OR (padma_status IS NULL AND memberships.id IS NOT NULL)").includes(:business).includes(:current_membership).includes(:current_membership => :installments).uniq 
