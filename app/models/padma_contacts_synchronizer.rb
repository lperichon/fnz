class PadmaContactsSynchronizer
  ...
  def sync
    ...
    if contacts.empty? || @business.synchronized_at.nil?
      ...
      padma_contacts = PadmaContact.search(select: [:first_name, :last_name, :local_status, :local_teacher], ids: padma_contact_ids, :where => {:updated_at =>  @business.synchronized_at}, per_page: padma_contact_ids.size, username: @business.owner.username, account_name: @business.padma_id)
    end
