module MembershipsHelper
  ...
  def table_contact_membership_link(contact, membership)
  	link_to(membership.present? ? t('memberships.overview.close_membership') : t('memberships.secondary_navigation.new_membership') , membership.present? ? business_membership_path(@business, membership) : new_business_membership_path(@business, :membership => {:contact_id => contact.id}))
  end	
