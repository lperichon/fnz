IntercomRails.config do |config|
  ...
  config.user.custom_data = {
    ...
    name: Proc.new { |user| user.name.present? ? user.name : user.drc_uid.split('.').join(' ').titleize },
  ...
  #
  config.company.current = Proc.new { @business }
  ...
  config.company.custom_data = {
    id: Proc.new { |business| business.padma_id },
    name: Proc.new { |business| nil },
    full_name: Proc.new{ |business| nil },
    enabled: Proc.new { |business| nil },
    created_at: Proc.new {|_| nil }, # created_at should only be sent by accounts-ws
    migrated_at: Proc.new { |business| nil },
    transactions_enabled: Proc.new {|business| business.transactions_enabled? }
  }
